  ...
  try {
    ...
    try {
      ...
      if (executionError && typeof executionError === 'object' && 'stderr' in executionError) {
        console.error(`Command execution stderr: ${String((executionError as any).stderr)}`);
      }
...
export async function getBatteryStatus(): Promise<string> {
  try {
    ...
    try {
      ...
      console.error("Terminal approach failed:", 
    ...
    if (error instanceof Error && 'stderr' in error) {
      console.error(`Stderr from failed command: ${String((error as any).stderr)}`);
    }
